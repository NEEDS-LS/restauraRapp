...
vignette: >
  %\VignetteIndexEntry{restauraRapp}
...
As informações cartográficas sobre o uso do solo podem ser de diversas fontes. O padrão é o dado também disponível na base de dados da [FBDS](https://geo.fbds.org.br/).
...
Os dados do CAR estão disponiveis na plataforma do [SICAR](https://car.gov.br/publico/imoveis/index) e o download é feito por estado. O Download destes dados devem ser feitos manualmente devido a presença de um "captcha" que impede o acesso ao link de download diretamente.
...
A função *st_read()* e a função de conversão de CRS *st_transform()* (no exemplo abaixo), devem ser executadas para carregar e alterar o CRS de todos os arquivos utilizados.

```{r, echo=TRUE, warning=FALSE, message=FALSE}
#carrega o arquivo de usos e cobertura do solo na memória do R
USO<-st_read("seu diretório de trabalho", layer="nome do arquivo")
#carrega o arquivo de massas d'água na memória do R
MDA<-st_read("seu diretório de trabalho", layer="nome do arquivo")
#carrega o arquivo de rios de margem simples (< 10 metros) na memória do R
RMS<-st_read("seu diretório de trabalho", layer="nome do arquivo")
#carrega o arquivo de rios de margem dupla (> 10 metros) na memória do R
RMD<-st_read("seu diretório de trabalho", layer="nome do arquivo")
#carrega o arquivo de nascentes na memória do R
NAS<-st_read("seu diretório de trabalho", layer="nome do arquivo")
#carrega o arquivo do CAR na memória do R
CAR<-st_read("seu diretório de trabalho", layer="nome do arquivo")
...
No caso de Campina do Monte Alegre, usaremos o EPSG 31982, referente ao SIRGAS 2000/ UTM zone 22s.
