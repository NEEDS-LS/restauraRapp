...
devtools::install_github("NEEDS-LS/restauRapp")
```
...
Para obter os dados de hidrografia e uso do solo padrões para o pacote (FBDS) basta executar a função *resapp_fbds_dados()*. Esta função recebe a sigla do estado a qual o município foco esta localizdo e o nome do município em caixa alta, separado por "_" quando necessário, como mostrado no exemplo a seguir.
...
#Para executar o tipo "df" é necessário todos os resultados da função gCARapp()
  dados.CARapp<-resapp_app_info(BURI_APP,BURI_OUT1,BURI_OUT2,tipo="df")
